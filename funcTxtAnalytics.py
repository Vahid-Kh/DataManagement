...
def add_fulltext(df):
    try:
        for i in df.index:
            patent_ID = df.loc[i, 'Publication Number']
            abstract = patent_fulltext(patent_ID)
            if abstract == "NaN":
                df.loc[i, "Fulltext"] = "Not able to extract abstract"
            else:
                df.loc[i, "Fulltext"] = abstract
            # print(abstract)
            time.sleep(0.95)
    except:

        # print("add_abstract Exception occured")
        df.loc[i, "Fulltext"] = df.loc[i, 'Publication Number']
